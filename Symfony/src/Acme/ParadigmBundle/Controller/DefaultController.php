...
class DefaultController extends Controller {
  ...
  public function indexAction(Request $request ){      
    ...
    $request->getSession()->set('auth', $user->get());             // PHP native: $_SESSION['auth'] = $user
    $request->getSession()->set('token', md5('bvjhgmbc'));

    // DEBUG
    // echo "<pre>".h::encode_php($user->get());die;
    // echo "<pre>".h::encode_php($request->cookies);die;
  ...
  public function cmsAction(Request $request){
      ...
      // DEBUG
      // echo "<pre>".h::encode_php($request->getSession());die;
